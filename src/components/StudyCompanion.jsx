...
// ─── Main Component ───────────────────────────────────────────────────────────
export default function StudyCompanion({ userData }) {
  ...
  const handleGenerate = async () => {
    ...
    try {
      ...
      if (response) {
        ...
        // Demo mode fallback
        const parsed = parseAIResponse(DEMO_NOTES);
  ...
  return (
    <div style={{ padding: '24px 28px', maxWidth: '1100px', margin: '0 auto', minHeight: '100vh' }}>
      ...
      {/* ── Main Layout ──────────────────────────────────────────────────── */}
      <div style={{ display: 'grid', gridTemplateColumns: studyKit ? '1fr 1.4fr' : '1fr', gap: '24px', alignItems: 'start' }}>
        ...
        {(isGenerating || studyKit) && (
          <div style={{ display: 'flex', flexDirection: 'column', gap: '16px', animation: 'fadeInUp 0.5s ease' }}>
            ...
            <div style={{
              ...
              {[
                { id: 'notes', label: '📝 Notes & Summary', count: null },
                { id: 'quiz',  label: '🧠 Interactive Quiz', count: studyKit?.quiz?.length || 0 },
              ].map(tab => {
                const active = activeSection === tab.id;
                return (
                  <button key={tab.id} onClick={() => setActiveSection(tab.id)} style={{
                    flex: 1, padding: '10px 16px', borderRadius: '10px', border: 'none',
                    background: active ? 'linear-gradient(135deg, rgba(124,58,237,0.15), rgba(99,102,241,0.1))' : 'transparent',
                    color: active ? '#818cf8' : 'var(--text-muted)',
              ...
              <div style={{
                ...
                <div style={{
                  ...
                  <span style={{
                    ...
                    background: 'rgba(99,102,241,0.1)', padding: '3px 10px', borderRadius: '10px'
                  }}>
                    {studyKit?.quiz?.length || 0} Questions
                  </span>
                </div>
                ...
                <div style={{ padding: '20px 24px' }}>
                  {isGenerating ? (
                    ...
                    <InteractiveQuiz questions={studyKit.quiz} />
                  ) : (
